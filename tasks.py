...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-webex runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-webex.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_webex")
...
def lint(c):
    ...
    """
    c.run("flake8 django-webex tests")
...
def release(c, bumpsize=''):
    ...
    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_webex.__version__))
    c.run("git push --tags")
